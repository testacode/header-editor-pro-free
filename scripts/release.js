...
async function main() {
  try {
    ...
    if (currentBranch !== 'main' && currentBranch !== 'master') {
      print(
        'yellow',
        `You're on branch '${currentBranch}'. Releases are typically made from main/master.`
      );
    ...
    let newVersion;
    // let releaseType; // Not used in this script

    switch (choice) {
      case '1':
        newVersion = patchVersion;
        // releaseType = 'patch'; // Not used after assignment
        break;
      case '2':
        newVersion = minorVersion;
        // releaseType = 'minor'; // Not used after assignment
        break;
      case '3':
        newVersion = majorVersion;
        // releaseType = 'major'; // Not used after assignment
        break;
      case '4':
        newVersion = await question('Enter custom version (e.g., 1.2.3): ');
        // releaseType = 'custom'; // Not used after assignment
        break;
      default:
    ...
    // Get release notes
    const releaseNotes =
      (await question('Enter release notes (optional, press Enter to skip): ')) ||
